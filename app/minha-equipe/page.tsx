...
const getStatus = (relatorio: RelatorioItem): StatusType => {
  ...
  if (relatorio.valido_ate) {
    const dataDisparo = new Date(
      `${relatorio.data_disparo}T${relatorio.hora_disparo}`
    );
...
export default function RelatoriosEquipePage() {
  ...
  const agruparRelatorios = useCallback(
    (
  ...
  const fetchSubordinados = useCallback(async (email: string) => {
    try {
      const encodedEmail = encodeURIComponent(email);
  ...
  const fetchRelatorios = useCallback(async () => {
    ...
    try {
      ...
      const allEmails = subordinados
        .map((sub) => sub.EMAIL_SUBORDINADO)
        .join(",");
    ...
    (relatoriosData: RelatoriosData) => {
      ...
      Object.values(relatoriosData).forEach((relatorios) => {
        relatorios.forEach((relatorio) => {
          ...
          else if (new Date() > new Date(relatorio.valido_ate || ""))
            expirados++;
          else pendentes++;
        });
      });
  ...
  const relatoriosAgrupados = agruparRelatorios(
    relatorios,
    filteredSubordinados
  );
        ...
        <div className="bg-white shadow-lg rounded-lg p-6">
          ...
          <div className="rounded-md border">
            <Table>
              ...
              <TableBody>
                {Object.entries(relatoriosAgrupados).map(
                  ([funcionario, datas]) =>
                    Object.entries(datas).map(([data, validacoes], index) => (
                      <TableRow
                        key={`${funcionario}-${data}`}
                            ...
                            <Tooltip>
                              ...
                              <TooltipContent>
                                {validacoes.manha?.hora_validacao
                                  ...
                                  ? `Válido até ${format(
                                      new Date(validacoes.manha.valido_ate),
                                      "HH:mm",
                                      { locale: ptBR }
                                    )}`
                                  : "Sem registro"}
                            ...
                            <Tooltip>
                              ...
                              <TooltipContent>
                                {validacoes.tarde?.hora_validacao
                                  ...
                                  ? `Válido até ${format(
                                      new Date(validacoes.tarde.valido_ate),
                                      "HH:mm",
                                      { locale: ptBR }
                                    )}`
                                  : "Sem registro"}
