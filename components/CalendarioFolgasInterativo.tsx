...
export default function CalendarioFolgasInterativo() {
  ...
  const salvarTodos = async () => {
    try {
      ...
      if (response.ok && data.sucesso) {
        // Marcar todos como confirmados
        setColaboradoresConfirmados(new Set(subordinados.map(s => s.chapa)))
        setColaboradoresPendentes(new Set())
        
        setMensagem({
          tipo: "sucesso",
          texto: `Todos os agendamentos foram salvos com sucesso!`,
        })
        setTimeout(() => setMensagem(null), 3000)
      } else {
      ...
      setMensagem({
        tipo: "erro",
        texto: "Erro ao salvar agendamentos. Tente novamente.",
      })
      setTimeout(() => setMensagem(null), 4000)
    } finally {
            ...
            {subordinadosFiltrados.length > 0 && (
              <div className="p-4 bg-gray-50 border-t flex justify-end">
                <Button
                  ...
                  size="lg"
                  className="bg-[#1e7b4f] hover:bg-[#165c3c] text-white px-6 py-2.5 font-semibold disabled:opacity-50"
                >
                  {salvandoGeral ? (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>Salvar Todos os Agendamentos</span>
                    </div>
                  )}
                </Button>
              </div>
          ...
          {subordinadosFiltrados.length > 0 && (
            <div className="mt-4">
              <Button
                ...
                disabled={salvandoGeral}
                size="lg"
                className="w-full bg-[#1e7b4f] hover:bg-[#166239] text-white py-3 font-semibold disabled:opacity-50"
              >
                {salvandoGeral ? (
